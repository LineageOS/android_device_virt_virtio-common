...
int setup_uinput_device(int *uinput_fd, struct input_absinfo *abs_x_info, struct input_absinfo *abs_y_info) {
    ...
    *uinput_fd = open("/dev/uinput", O_WRONLY | O_NONBLOCK);
    ...
    // Set the ABS_X and ABS_MT_POSITION_X range based on the source device's ABS_X
    memset(&abs_setup, 0, sizeof(abs_setup));
    ...
    // Set the ABS_Y and ABS_MT_POSITION_Y range based on the source device's ABS_Y
    memset(&abs_setup, 0, sizeof(abs_setup));
...
void send_multitouch_event(int uinput_fd, int pressed, int tracking_id, int x, int y) {
...
void send_key_event(int uinput_fd, unsigned int code, int value) {
    struct input_event ev;
...
void handle_event(int uinput_fd, int type, int code, int value, int *x, int *y) {
        ...
        if (ret > 0) {
            for (int i = 0; i < ret; ++i) {
                if (events[i].events & EPOLLIN) {
                    ...
                    if (rc == sizeof(ev)) {
                        handle_event(uinput_fd, ev.type, ev.code, ev.value, &x, &y);
