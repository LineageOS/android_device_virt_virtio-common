...
1. Boot into latest AOSP main GSI
...
3. Push required files to `/data/local/tmp`
...
4. Create userdata image (Note that `fallocate` command would not work for this!)

`dd if=/dev/zero of=/data/local/tmp/disk-vdb.img bs=1M count=2048`

5. Write VM configuration to `/data/local/tmp/vm_config.json`
